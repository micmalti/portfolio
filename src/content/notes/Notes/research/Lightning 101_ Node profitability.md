...
There is the expectation that one would be making sats straight away, from the moment the Bitcoin lightning node is set up with little to no maintainance required. This is actually far from the truth, in most cases. Indeed, it's the reverse: it's difficult to run a profitable routing service.

First of all, there's an element of chance involved, as people try to compete for the same routes which you will be trying to get. Also, you wouldn't know exactly when you'll be assigned a route. The part which makes it "fun" is that your tweaks will directly impact your success.

A realistic target would be a yield of 2%.

## Channel management

Since the network changes on a daily basis, channel management constantly requires human intervention. According to the speakers, having an automated script simply isn't a good enough substitute for manual intervention, particularly when it comes to finding the optimum balance between the rebalancing cost and the channel fee, and which connections will provide the most volume per amount of sats.

Be extremely mindful of on-chain fees, on-channel fees (loop-outs), opening and closing channel fees, and liquidity fees, because they add up very quickly and impact the profitability of the node over a long period of time.

## Liquidity

The mempool is where all the valid transactions await confirmation by the Bitcoin network. A high mempool size indicates more network traffic which will result in longer average confirmation time and higher priority fees. 

:::note
Since 2021, the average transaction fee has been trending downwards, except for the period around the fourth halving.
:::

## Rebalancing 

With rebalancing, it's very easy to get into trouble. In order to make sats, you need a payment flow. However, each time a payment flows through a channel, that channel becomes out of balance. As of mid-2021, the available tooling for executing a rebalancing was still being worked upon and cannot be relied upon at all times. Such tools include RTL and Balance of Satoshis. 

There are different schools of thought when it comes to rebalancing. Some believe a channel needs to be perfectly balanced but this ends up raising the fees incurred which may result in a loss of profitability. Others think that an agressive fee readjustment, rather than active rebalancing, is a better strategy which leaves the rebalancing to the flows. Some do both. 
